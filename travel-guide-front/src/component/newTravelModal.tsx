...
const NewTravelModal: React.FC<IPropsGlobal> = props => {
    ...
    if (travel && travel._id !== "" && theFile && theFile !== "") {
      ...
      fetch("http://localhost:3000/api/travels/" + travel._id + "/photo", {
        ...
        if (res.ok) {
          ...
          res.text().then((newPhoto: string) => {
            travel.photo = newPhoto;
            props.callback(travel);
          });
        } else {
  ...
  return (
    <div className="margins height100">
      ...
      <div className="row">
        ...
        <p>&nbsp;</p>
        <div className="input field col s6">
          <Button className="modal-close waves-effect waves-light  mybuttonnav back" onClick={addTravel}><i className="small material-icons left">library_add</i>Crear Experiencia</Button>
        </div>
        <div className="input field col s6">
